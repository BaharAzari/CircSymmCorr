...
class CCL_(nn.Module):
    ...
    def forward(self, x):  # x: N x C_in x H x W
        ...
        dims = x.shape[-2:]
        dct = LinearDCT(dims[0], 'dct').to(x.device)
        idct = LinearDCT(dims[0], 'idct').to(x.device)
...
class CCL__(nn.Module):
    def __init__(self, c_in, c_out, kernel, stride):
        ...
        K = np.sqrt(1 / (c_in * kernel[0]))
        init = (2 * torch.rand(c_out, c_in, kernel[0]) - 1) * K
        self.weight_dct = nn.Parameter(init)
        K = np.sqrt(1 / (c_in * kernel[1]))
        init = (2 * torch.rand(c_out, c_out, kernel[1]) - 1) * K
        self.weight_fft = nn.Parameter(init)
    ...
    def forward(self, x):  # x: N x C_in x H x W
        ...
        w = dct(F.pad(self.weight_dct,[0, dims[0] - self.kernel[0]])).unsqueeze(-1)
        h = (x * w).sum(2)
        h = idct(h.permute(0, 1, 3, 2)).permute(0, 1, 3, 2)

        h = torch.fft.rfft(h).unsqueeze(1)
        w = torch.fft.rfft(self.weight_fft, dims[1]).unsqueeze(-2)
...
class Net_CNN(nn.Module):
    def __init__(self):
        ...
        #self.conv5 = nn.Conv2d(256, 10, 1, 1)
...
class Net_CCL(nn.Module):
    def __init__(self):
        ...
        # self.conv1 = CCL__(3, 64, 5, 1)
        # self.conv2 = CCL__(64, 64, 5, 1)
        # self.conv3 = CCL__(64, 64, 3, 1)
        # self.conv4 = CCL__(64, 64, 3, 1)
        # #self.conv5 = nn.Conv2d(256, 10, 1, 1)
        ...
        self.conv1 = CCL(3, 128, 5, 1)
        self.conv2 = CCL(128, 128, 5, 1)
        self.conv3 = CCL(128, 128, 3, 1)
        self.conv4 = CCL(128, 256, 3, 1)
        #self.conv5 = nn.Conv2d(256, 10, 1, 1)
        self.fc1 = nn.Linear(256, 120)
        self.fc2 = nn.Linear(120, 84)
        self.fc3 = nn.Linear(84, 10)
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        #data = data.roll(np.random.randint(0,16,1)[0],-1)
        optimizer.zero_grad()
        output = model(data)
...
def main():
    ...
    dataset2 = datasets.CIFAR10(root='./data', train=False,
                                download=True, transform=transform)
    ...
    if args.train:
        #model.load_state_dict(torch.load("cfar10_%s.pt" % which))
        for epoch in range(1, args.epochs + 1):
            train(args, model, device, train_loader, optimizer, epoch)
            test(model, device, test_loader, roll=0)

            if args.save_model:
                torch.save(model.state_dict(), "cfar10_%s.pt" %which)
    else:
        model.load_state_dict(torch.load("cfar10_%s.pt" %which))
        for roll in range(22):
